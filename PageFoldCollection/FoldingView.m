...
- (void)addBottomView
{
    ...
    bottomView = [[UIImageView alloc] initWithFrame:CGRectMake(0.f,
                                                                            CGRectGetMidY(self.bounds),
                                                                            CGRectGetWidth(self.bounds),
                                                                            CGRectGetMidY(self.bounds))];
    bottomView.image = image;
    bottomView.layer.anchorPoint = CGPointMake(0.5, 0);
    bottomView.layer.position = CGPointMake(CGRectGetMidX(self.frame), CGRectGetMidY(self.frame));
...
- (void)addGestureRecognizers
{
    ...
    [bottomView addGestureRecognizer:panGestureRecognizer2];
}

- (void)poke
{
    [self rotateToOriginWithVelocity:5 WithView:_topView];
}
...
- (void)handlePan:(UIPanGestureRecognizer *)recognizer
{
    ...
    NSLog(@"rotation x is %f",[[self.topView.layer valueForKeyPath:@"transform.rotation.x"] floatValue]*180);
    ...
    if (recognizer.state == UIGestureRecognizerStateEnded ||
        ...
        [self rotateToOriginWithVelocity:0 WithView:_topView];
    }
}
...
- (void)handlePan2:(UIPanGestureRecognizer *)recognizer
{
    ...
    NSLog(@"rotation x is %f",[[bottomView.layer valueForKeyPath:@"transform.rotation.x"] floatValue]*180);
    
    if ([[bottomView.layer valueForKeyPath:@"transform.rotation.x"] floatValue] < M_PI_2) {
        self.backView.alpha = 1.0;
        [CATransaction begin];
        [CATransaction setValue:(id)kCFBooleanTrue
                         forKey:kCATransactionDisableActions];
    ...
    if ([self isLocation:location inView:self]) {
        CGFloat conversionFactor = M_PI / (self.initialLocation);
        POPBasicAnimation *rotationAnimation = [POPBasicAnimation animationWithPropertyNamed:kPOPLayerRotationX];
        
        rotationAnimation.duration = 0.01;
        rotationAnimation.toValue = @((self.initialLocation - location.y)*conversionFactor);
        [bottomView.layer pop_addAnimation:rotationAnimation forKey:@"rotationAnimation2"];
    } else {
        recognizer.enabled = NO;
        recognizer.enabled = YES;
    }
    ...
    if (recognizer.state == UIGestureRecognizerStateEnded ||
        recognizer.state == UIGestureRecognizerStateCancelled) {
        [self rotateToOriginWithVelocity:0 WithView:bottomView];
    }
}
